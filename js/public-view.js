class PublicTournamentViewer {
    ...
    initFirebase() {
        ...
        const firebaseConfig = {
            apiKey: "AIzaSyBl7zX8Q8X8Q8X8Q8X8Q8X8Q8X8Q8X8Q8X",
            authDomain: "tourneyproapp.firebaseapp.com",
            projectId: "tourneyproapp",
            storageBucket: "tourneyproapp.appspot.com",
    ...
    setupEventListeners() {
        ...
        // Tournament filter
        document.getElementById('tournament-filter').addEventListener('change', (e) => {
            this.filterMatches(e.target.value);
        });

        // Standings filter
        document.getElementById('standings-tournament-filter').addEventListener('change', (e) => {
            this.showStandings(e.target.value);
        });

        // Players filters
        document.getElementById('players-tournament-filter').addEventListener('change', () => {
            this.filterPlayers();
        });
        document.getElementById('players-club-filter').addEventListener('change', () => {
            this.filterPlayers();
        });
        ...
        this.data.tournaments.forEach(tournament => {
            ...
            card.innerHTML = `
                ...
                <div class="tournament-stats">
                    ...
                    <div class="stat-item">
                        <span class="stat-value">${this.getTotalGoals(tournament.id)}</span>
        ...
        matches.forEach(match => {
            ...
            card.innerHTML = `
                <div class="match-teams">
                    <div class="team">
                        <img src="${homeTeam?.logo || 'https://via.placeholder.com/40'}" alt="${homeTeam?.name}" class="team-logo">
                    ...
                    <div class="team">
                        ...
                        <img src="${awayTeam?.logo || 'https://via.placeholder.com/40'}" alt="${awayTeam?.name}" class="team-logo">
                    </div>
                </div>
                <div class="match-info">
                    <div>${this.formatDate(match.date)}</div>
                    <div style="font-size: 0.8rem; opacity: 0.8;">Rodada ${match.round}</div>
                </div>
            `;
    ...
    showStandings(tournamentId) {
        ...
        if (!tournamentId) {
            container.innerHTML = '<p style="color: white; text-align: center;">Selecione um torneio para ver a classificação</p>';
            return;
        }
                    ...
                    <tbody>
                        ${standings.map((team, index) => `
                            <tr>
                                ...
                                <td class="team-cell">
                                    <img src="${team.logo || 'https://via.placeholder.com/30'}" alt="${team.name}" class="team-logo">
                                    ${team.name}
                                </td>
                                <td>${team.matches}</td>
                                <td>${team.wins}</td>
    ...
    setupPlayerFilters() {
        const tournamentFilter = document.getElementById('players-tournament-filter');
    ...
    filterPlayers() {
        const tournamentId = document.getElementById('players-tournament-filter').value;
        ...
        if (tournamentId) {
            ...
            players = players.filter(p => tournamentClubs.some(c => c.id == p.clubId));
        }
        ...
        players.forEach(player => {
            ...
            card.innerHTML = `
                <div class="player-header">
                    <img src="${player.photo || 'https://static.flashscore.com/res/image/empty-face-man-share.gif'}" alt="${player.name}" class="player-photo">
                    <div class="player-info">
                        <h4>${player.name}</h4>
                        <div class="player-club">${club?.name || 'Sem clube'}</div>
    ...
    showTournamentDetails(tournamentId) {
        this.currentTournament = this.data.tournaments.find(t => t.id == tournamentId);
        if (!this.currentTournament) return;

        document.getElementById('tournament-modal-name').textContent = this.currentTournament.name;
        document.getElementById('tournament-modal-game').textContent = this.currentTournament.game;
        document.getElementById('tournament-modal-game').className = `game-badge ${this.currentTournament.game.toLowerCase()}`;
        document.getElementById('tournament-modal-dates').textContent = 
            `${this.formatDate(this.currentTournament.startDate)} - ${this.formatDate(this.currentTournament.endDate)}`;
    ...
    loadTournamentOverview() {
        const clubs = this.data.clubs.filter(c => c.tournamentId == this.currentTournament.id);
        const matches = this.data.matches.filter(m => m.tournamentId == this.currentTournament.id);
        ...
        document.getElementById('overview-tab').innerHTML = `
            <div class="tournament-overview">
                <div class="overview-stats">
                    ...
                    <div class="stat-card">
                        <h4>Total de Gols</h4>
                        <div class="stat-number">${this.getTotalGoals(this.currentTournament.id)}</div>
                    </div>
                </div>
                <div class="clubs-grid">
                    ${clubs.map(club => `
                        <div class="club-item">
                            <img src="${club.logo || 'https://via.placeholder.com/40'}" alt="${club.name}">
                            <span>${club.name}</span>
                        </div>
                    `).join('')}
                </div>
    ...
    loadTournamentMatches() {
        const matches = this.data.matches.filter(m => m.tournamentId == this.currentTournament.id && m.status === 'finished');
                ...
                ${matches.map(match => {
                    ...
                    return `
                        <div class="match-item">
                            <div class="match-date">${this.formatDate(match.date)} - Rodada ${match.round}</div>
                    ...
                    <tbody>
                        ${standings.map((team, index) => `
                            <tr>
                                <td>${index + 1}</td>
                                <td class="team-cell">
                                    <img src="${team.logo || 'https://via.placeholder.com/20'}" alt="${team.name}" class="team-logo">
                                    ${team.name}
                                </td>
                                <td><strong>${team.points}</strong></td>
                                <td>${team.matches}</td>
                ...
                <div class="scorers-list">
                    ${topScorers.slice(0, 10).map((scorer, index) => `
                        <div class="scorer-item">
                            ...
                            <img src="${scorer.photo || 'https://static.flashscore.com/res/image/empty-face-man-share.gif'}" alt="${scorer.name}" class="player-photo">
    ...
    showPlayerProfile(playerId) {
        ...
        document.getElementById('player-modal-photo').src = player.photo || 'https://static.flashscore.com/res/image/empty-face-man-share.gif';
        document.getElementById('player-modal-name').textContent = player.name;
        document.getElementById('player-modal-position').textContent = player.position;
        document.getElementById('player-modal-position').className = `position-badge ${player.position?.toLowerCase()}`;
        document.getElementById('player-modal-club').textContent = club?.name || 'Sem clube';
        document.getElementById('player-modal-age').textContent = player.age ? `${player.age} anos` : '';
    ...
    getTopScorers(tournamentId) {
        ...
        const scorers = players.map(player => {
            ...
            matches.forEach(match => {
                if (match.events) {
                    match.events.forEach(event => {
                        if ((event.playerId == player.id || event.player === player.name) && event.type === 'Gol') {
                            goals++;
                        }
                    });
                }
